...
COPY cce-step-sla-service/gradle/ cce-step-sla-service/gradle/
COPY cce-step-sla-service/gradlew cce-step-sla-service/build.gradle cce-step-sla-service/settings.gradle cce-step-sla-service/

WORKDIR /workspace/cce-step-sla-service
RUN chmod +x gradlew ../cce-common-util/gradlew \
    && ./gradlew dependencies --no-daemon 2>/dev/null || true

COPY cce-step-sla-service/src/ src/
